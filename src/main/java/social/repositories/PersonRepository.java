...
public interface PersonRepository extends CrudRepository<Person, Long> {
    ...
    @Query("SELECT p FROM Person p WHERE p.id NOT IN (SELECT DISTINCT p.id FROM Person p JOIN Friendship f on (:personid IN (f.friend.id, f.friendship.id) AND p.id IN (f.friend.id, f.friendship.id))) AND p.id <> :personid")
